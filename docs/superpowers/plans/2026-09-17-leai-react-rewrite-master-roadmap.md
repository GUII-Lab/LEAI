...
- [x] **M0.6 Build the shared shell.** Implement environment bar, account rail, course navigation, page header, responsive Sheet navigation, and stable content geometry. The environment gate supplies read-only mismatch behavior; feature-level loading/error boundaries are added with their data-owning screens.
